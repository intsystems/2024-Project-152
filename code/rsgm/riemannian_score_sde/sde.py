...
def sym_matrix_to_vector(mat, n):
    indices = jnp.triu_indices(n)
    return mat[indices]

def vector_to_sym_matrix(vec, n):
    # (n*(n+1)/2,) -> (n, n)
    ...
    print(mat.shape)
    print(vec.shape)
    mat = mat.at[indices].set(vec)
    mat = mat.at[indices[::-1]].set(vec)
    return mat
...
class Langevin(SDE):
    ...
    def __init__(
        ...
        self.N = N
        # self._n_for_vec_ops = manifold.n 
    ...
    def drift(self, x, t):
        ...
        def drift_per_sample(x_mat):
            x_vec = sym_matrix_to_vector(x_mat, n) # (n*(n+1)/2,)
            # print("x_vec shape = ", x_vec.shape)
            
            grad_vec = self.limiting.grad_U(x_vec) # (n*(n+1)/2,)
            
            fixed_grad_vec = fixed_grad(grad_vec)
            
            grad_mat = vector_to_sym_matrix(fixed_grad_vec, n) # (n, n)
            
            return -0.5 * grad_mat

        drift_fn = jax.vmap(drift_per_sample)
        
        # drift_fn = jax.vmap(lambda x: -0.5 * fixed_grad(self.limiting.grad_U(x)))
        beta_t = self.beta_schedule.beta_t(t)
        beta_t_reshaped = jnp.reshape(beta_t, (-1, 1, 1))
        # print("beta shape = ", beta_t_reshaped.shape)
        drift = beta_t_reshaped * drift_fn(x)
        return drift
...
class VPSDE(Langevin):
    def __init__(self, beta_schedule, manifold=None, **kwargs):
        super().__init__(beta_schedule, manifold, **kwargs)
        print("dim = ", manifold.dim)
